...
In 2025, two documents of the Government of India described the same policy.

The first, a press release from the Ministry of Petroleum and Natural Gas, announced that India had achieved 20 per cent ethanol blending in petrol — five years ahead of schedule — and tallied the winnings: lakhs of crores in foreign exchange saved, lakhs of crores paid to farmers, crores of tonnes of carbon dioxide avoided. It was, by any reading, a policy triumph.

The second, the Economic Survey 2025-26, described "an emerging tension between Aatmanirbharta in energy and Aatmanirbharta in food," warned that farmers were abandoning pulses and oilseeds for maize, and called the trend an early warning signal.

Both are true. This post is an attempt to hold them together.

I want to be clear at the outset about what this is not. It is not an argument that ethanol blending was a mistake. India imports something like 85 to 88 per cent of its crude oil, and a country in that position is entitled to try almost anything. Nor is it a repetition of the viral claim that a litre of ethanol costs ten thousand litres of water — a number that is true for one feedstock and badly misleading for the others, as we will see.

It is an attempt to do the accounting properly. Every litre of ethanol in an Indian petrol tank came from somewhere: from cane, from maize, from rice out of the Food Corporation of India's godowns. This blog has spent its life on the question of what our food system can and cannot absorb. E20 is a very large withdrawal from that system, and it deserves a balance sheet rather than a press release.

Let's dive into some data.

# What India actually achieved

The scale of the ramp-up is genuinely remarkable, and it would be dishonest to open with the criticisms.

::: {#fig-blending}
```{r fig-blending}
# read_csv("data/ethanol_blending_trajectory.csv") |>
#   ggplot(aes(esy_year, blending_percent, group = 1)) +
#   geom_line() + geom_point() +
#   labs(x = "Ethanol Supply Year", y = "Blending (%)")
```

Ethanol blending in Indian petrol, 2013-14 to 2024-25
:::

Blending rose from 1.53 per cent in 2013-14 to roughly 19 to 20 per cent by the 2024-25 ethanol supply year. Volumes supplied to oil marketing companies went from 38 crore litres to somewhere between 900 and 1,016 crore litres over the same period — a factor of about twenty-five. Very few Indian industrial programmes have scaled like that. Since April 2026, E20 is the base grade: essentially all petrol sold in the country carries 20 per cent ethanol at a minimum octane rating of 95.

The official ledger, as of the most recent government factsheets, reads roughly as follows. About ₹1.90 lakh crore in foreign exchange saved between 2014-15 and mid-2026, by substituting some 310 lakh metric tonnes of imported crude. Over ₹1.6 lakh crore paid to farmers. Somewhere between 790 and 930 lakh metric tonnes of carbon dioxide avoided. Sugar mills, chronically unable to pay cane arrears, got a second revenue stream. Distilleries created rural jobs in Uttar Pradesh, Maharashtra and Bihar.
...
NITI Aayog's own 2021 roadmap put the annual saving from a fully successful E20 programme at about four billion US dollars. India's oil import bill in FY 2024-25 was roughly 137 billion dollars.

::: {#fig-offset}
```{r fig-offset}
# read_csv("data/import_offset_scale.csv") |>
#   ggplot(aes(reorder(item, value_usd_billion), value_usd_billion)) +
#   geom_col() + coord_flip() +
#   labs(x = NULL, y = "US$ billion")
```
...
The views expressed in this blog are personal and not my employer's.
